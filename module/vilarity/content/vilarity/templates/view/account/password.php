...
<ss:template name="#content:vilarity/templates/view/account/password"
...
<div class="uk-height-1-1 uk-overflow uk-padding-small" style="position: relative;">
  <div style="position: relative;">
    ...
    <div v-else="">
      <h3><!--<a is="router-link" v-bind:to="ss.route.basePath">All Accounts</a> > --><span v-if="account.user.name != ''">{{ account.user.name }} (</span><a
          is="router-link" v-bind:to="ss.route.parentPath">{{ account.user.username }}</a><span
          v-if="account.user.name != ''">)</span></h3>
      ...
      <div class="uk-width-1-3@xl uk-width-1-2@l uk-width-2-3@s uk-width-1-1">
        <p>Pick a strong password that is 8 to 32 characters
          of upper case, lower case, numbers, and symbols.</p>
        <user-form-user-password v-bind:user="account.user" />
      </div>
    </div>
  </div>
</div>
