...
<ss:template name="#content:vilarity/templates/view/account"
...
<div class="uk-height-1-1 uk-overflow uk-padding-small" style="position: relative;">
  <div style="position: relative;">
    ...
    <div v-else="">
      <h3><!--<a is="router-link" v-bind:to="ss.route.parentPath">All Accounts</a> > --><span v-if="account.user.name != ''">{{ account.user.name }} (</span><a
          is="router-link" v-bind:to="ss.route.currentPath">{{ account.user.username }}</a><span
	  ...
	  <vilarity-form-account-properties mode="view" v-bind:account="account" v-bind:user="account.user" />
      </div>
    </div>
  </div>
</div>
